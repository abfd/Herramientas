...
import xmlCajaRural.ValoracionHistoricas;
...
public class envioHistoricas 
{
    ...
    {//EN EL EXCEL COLUMNA 1 = EXPEDIENTE COLUMNA 2 = ESTADO ENVIAR/NO ENVIAR
         ...
         try
         {
            ...
            sUrlExcel = "/data/informes/CajaRural/historicas/historicas.xls";
            ...
            {                                                
                ...
                xmlCajaRural.XMLTasacionRural nwValoracion = new xmlCajaRural.XMLTasacionRural(numexp);         
                //xmlCajaRural.ValoracionHistoricas nwValoracion = new xmlCajaRural.ValoracionHistoricas(numexp,1);                  
                PropertyConfigurator.configure("/data/informes/CajaRural/historicas/" + "Log4j.properties");   
                if (nwValoracion.estadoOK) 
                {                    
                    //if (nwValoracion.masDe1Finca) logger.info("Tasacion con mas de 1 finca registral generada: "+numexp);
                    logger.info("Tasación generada: "+numexp);
                }
                else
                {
                    logger.error("Error: "+numexp+". Descripción: "+nwValoracion.descError);
                    try
                    {
                        Utilidades.Correo.sendMail("valtecnic.com", "informatica@valtecnic.com", "abfd@valtecnic.com", "", "", numexp, nwValoracion.descError, true);
                    }
                    catch(Exception e)
                    {
                        logger.error("Excepción envio mail: "+e.toString());
                    }
                }
                nwValoracion = null;
                try
